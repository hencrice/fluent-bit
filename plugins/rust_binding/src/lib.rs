...
// bindings.rs is automatically generated by the rust-bindgen
include!(concat!(env!("OUT_DIR"), "/bindings.rs"));

// All the following types and tests were initially generated when build.rs
// includes the following 3 lines:
...
//    The main reason for doing this is because:
//      a) From C's perspective, there's no difference between *mut c_char and *const c_char
//      b) In order to expose a registration point from the Rust plugin (just like all the other C plugins),
//         there needs to be one globally mutable variable. And in Rust, such variable must be declared as
//         "static", meaning that the variable is valid as long as the process is alive. And one of the
//         restrictions of static variables is that values have to be constant.
...
pub struct flb_rust_stdout {
    ...
    pub json_date_key: *const ::std::os::raw::c_char,
...
// Fluent-bit registers plugins by looking up a struct variable defined in each plugin.
// The variable has to adhere to specific naming convention and is globally mutable, which
// is inherently unsafe. Fortunately, fluent-bit only reads from these global variables,
// so we assure the Rust compiler that these types are "thread-safe".
